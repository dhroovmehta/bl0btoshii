...
| 2026-02-17 | **Alerting coverage for 3 uncovered paths** | Global `on_command_error` handler sends unhandled command exceptions to #errors. Platform publishing exceptions now alert #errors via `notify_error`. Episode index read failure logs warning instead of silent `pass`. |
| 2026-02-17 | **Character positioning: LLM prompt + fallback + overlap fix** | Root cause: LLM was never shown valid position names (variable built but not passed to template), so it hallucinated names like "center_left" that don't exist. Fix: (1) `prompts.py` now passes position names in `location_description`. (2) Template explicitly instructs LLM to use ONLY listed positions. (3) `sprite_manager.py` fallback is now location-aware (picks first valid position for the location instead of hardcoded y=1300). (4) New `resolve_scene_positions()` prevents overlap by assigning characters to distinct available positions. (5) `validator.py` now rejects scripts with invalid position names. (6) 5 new TDD tests in `test_character_positioning.py` (23 total). |
| 2026-02-17 | **Missing SFX assets blocking video generation** | `door_burst.wav` and `menu_select.wav` were listed in `VALID_SFX` and the LLM prompt template as available sounds, but the actual WAV files never existed in `assets/sfx/`. The asset check in `check_asset_availability()` correctly blocked video generation when the LLM used `door_burst` in a script. Created both missing files. |
| 2026-02-17 | **SFX/music validation was dead code** | `VALID_SFX` and `VALID_MUSIC` lists existed in `validator.py` but were never checked during `validate_script()`. The LLM could use any SFX name and it would pass validation, only to fail at the asset check stage. Added SFX and music name validation to the script validation loop so bad names are caught during generation (with retries) instead of blocking at asset check. |
| 2026-02-17 | **Storyboard renderer positioning bug** | `src/storyboard/renderer.py` used `get_character_position()` directly with raw position names from the script, bypassing the `resolve_scene_positions()` anti-overlap logic. Storyboard previews showed characters at fallback positions (potentially overlapping) even when the video renderer would place them correctly. Fixed to use the same `resolve_scene_positions()` path as `scene_builder.py`. |
| 2026-02-17 | **Audio mixer silent failures** | `src/audio_mixer/mixer.py` silently skipped missing SFX and blip files with no log output. Added warning logs so missing audio assets surface in process output instead of producing silent videos with no explanation. |
